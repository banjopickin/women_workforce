...
The processed survey data contains 4469 female respondents with children under thirteen years old and 144 survey questions ranging from employment status to political views. This data is passed to the random forest model with employment status as the predicted variable. First, forty important features the first  random forest modeling were selected as independent variables to fit the second round  of modeling. After a grid search, the second round random forest model yielded a 0.81 roc_auc score.
...
Next, we need to find out common contribution paths within each target label. Using Kmeans to cluster a feature contribution matrix is a reasonable approach. However, the matrix has 40 columns, so before clustering, PCA is necessary for dimension reduction.
...
To determine how many Ks (# of clusters) are best for kmeans. a function is written to check kmeans' cluster performance. It takes a range of Ks and makes a for loop. In each loop it generates k clusters using kmeans, concatenates cluster id to 'employed' and 'correct' columns to make a new data frame. Then. it groups the data frame by cluster id, calculates employed rate, and corrects that rate by taking means.
If the employed rate is higher than 0.7 or lower than 0.3, then we can say this cluster successfully grouped into employed or unemployed populations, so this cluster yields pure populations of interest. If the correct rate is higher than 0.7, we can say this cluster has high accuracy. The clusters with pure population and with high accuracy are ones we want. The percentage of good cluster rates for each Kmeans is calculated and plotted against the number of Ks.
...
Then the plot averaged silhouette scores against K, and it suggested that it yields the highest silhoutte Score (0.52) when K is 6. Therefore we set n_cluster at six to conduct Kmeans.
